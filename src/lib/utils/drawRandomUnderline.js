...
const svgVariants = [
	...
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 26C42 10 78 34 118 18C158 8 198 30 238 14C268 6 292 22 305 17" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 32C28 18 52 36 82 22C112 8 142 30 172 16C202 4 232 28 262 12C282 6 298 26 305 21" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 30C48 22 92 36 136 26C180 18 224 34 268 24C290 18 305 28 305 26" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 12C38 28 72 8 108 24C144 38 180 10 216 22C252 34 288 6 305 16" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 28C22 14 40 32 58 18C76 8 94 30 112 16C130 6 148 28 166 14C184 8 202 26 220 18C238 12 256 30 274 20C292 14 305 26 305 24" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
	`<svg width="310" height="40" viewBox="0 0 310 40" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 18C80 32 155 10 230 26C270 32 300 14 305 22" stroke="currentColor" stroke-width="10" stroke-linecap="round"/></svg>`,
];
...
function bindDrawLines(root, gsap) {
	...
	root.querySelectorAll('[data-draw-line]').forEach((container) => {
		...
		const onEnter = () => {
			...
			nextIndex = ((nextIndex ?? 0) + 1) % svgVariants.length;
		};
...
export async function initDrawRandomUnderline(root) {
	...
	if (!gsap?.plugins?.drawSVG) return () => {};

	return bindDrawLines(root, gsap);
}
